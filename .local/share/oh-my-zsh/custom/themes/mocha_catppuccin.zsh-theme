...
if [[ $TERM = (*256color|*rxvt*|*xterm-kitty*) ]]; then
  turquoise="%{${(%):-"%B%F{004}"}%}"
  orange="%{${(%):-"%B%F{172}"}%}"
  purple="%{${(%):-"%B%F{141}"}%}"
  hotpink="%{${(%):-"%B%F{001}"}%}"
...
setopt prompt_subst
PROMPT="${purple}%n%{$reset_color%}@${white}%m%{$reset_color%} in ${limegreen}%~%{$reset_color%}\$(virtualenv_prompt_info)\$(ruby_prompt_info)\$vcs_info_msg_0_${orange} 󰅂%{$reset_color%} "
